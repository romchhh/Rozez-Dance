    ...
    <section className="bg-ivory">
      <div className="max-w-site mx-auto px-6 md:px-10 py-24 md:py-32">
        <div className="mb-16 reveal">
          <h2 className="font-display text-[clamp(40px,6vw,80px)] leading-none tracking-wider text-noir">
            THIS PROGRAM IS FOR YOU
            <br />
            <span className="text-noir/30 text-[0.5em] tracking-[6px] font-body font-light">IF YOU WANNA</span>
          </h2>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-noir/10">
          {LEVEL_GOALS.map((level, idx) => (
            <div key={level.title} className="bg-white/60 p-10 group relative overflow-hidden">
              {/* Hover glow */}
              ...
              <h3 className="font-display text-2xl tracking-widest text-noir mb-8">
                ...
                {level.goals.map((g) => (
                  <div key={g.num} className="group/item">
                    <div className="text-pink font-mono text-xs tracking-widest mb-1.5">/{g.num}</div>
                      <div className="text-noir/60 text-sm leading-snug">{g.text}</div>
                  </div>
                ))}
              </div>
            </div>
